...
export default function ContactPage() {
  ...
  return (
    <div className='flex flex-col w-full gap-4'>
            ...
            <CardContent className='flex-1 flex flex-col'>
              ...
              <div className='flex gap-2 mb-4'>
                <Select value={sort} onValueChange={setSort}>
                  <SelectTrigger className='w-full transition-all duration-200 hover:scale-105'>
              ...
              <ScrollArea className='flex-1'>
                {filteredUsers.map((user) => (
                  <div
                    key={user.id}
                    className={`flex items-center gap-3 p-2 hover:bg-muted rounded cursor-pointer transition-all duration-200 hover:scale-105 ${
                ...
                <div className='space-y-2'>
                  {messages.map((msg) => (
                    <div key={msg.id} className={`flex ${msg.sender === 'me' ? 'justify-end' : 'justify-start'} animate-fade-in`}>
                      <div
                        className={`p-2 rounded-lg max-w-xs transition-all duration-200 hover:scale-105 ${
      ...
      <section className='bg-muted p-4 border-t transition-all duration-300 hover:bg-muted/80 rounded-lg'>
        <div className='text-center'>
          <h3 className='text-xl font-semibold mb-4 flex items-center justify-center gap-2'>
            <MessageCircle className='w-5 h-5' /> Conectează-te pentru Tranzacții
          </h3>
          ...
          <Card className='transition-all duration-200 hover:scale-[1.02] hover:shadow-xl'>
            <CardHeader>
              <CardTitle className='flex items-center justify-center gap-2'>
                <MapPin className='w-6 h-6' /> Locuri de Întâlnire
              </CardTitle>
            </CardHeader>
            <CardContent>
              ...
              <p className='text-sm text-muted-foreground mt-4'>Vezi locațiile pentru întâlniri sigure. Clic pe markere pentru detalii.</p>
            </CardContent>
          </Card>
        </div>
      </section>
