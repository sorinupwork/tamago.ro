...
export const auto = {
  ...
  buySchema: z.object({
    title: z.string().min(1, 'Titlul este obligatoriu'),
    description: z.string().min(10, 'Descrierea trebuie să aibă cel puțin 10 caractere'),
    minPrice: z.string().min(1, 'Prețul minim este obligatoriu').regex(/^\d+([.,]\d+)?$/, { message: 'Prețul minim trebuie să fie un număr pozitiv' }),
    maxPrice: z.string().min(1, 'Prețul maxim este obligatoriu').regex(/^\d+([.,]\d+)?$/, { message: 'Prețul maxim trebuie să fie un număr pozitiv' }),
    currency: z.enum(['EUR', 'USD', 'RON'], { message: 'Moneda trebuie să fie EUR, USD sau RON' }),
    location: z.string().min(1, 'Locația este obligatorie'),
    features: z.string().optional(),
    status: z.string().min(1, 'Statusul este obligatoriu'),
    fuel: z.string().min(1, 'Combustibilul este obligatoriu'),
    minMileage: z.string().min(1, 'Kilometrajul minim este obligatoriu').regex(/^\d+([.,]\d+)?$/, { message: 'Kilometrajul minim trebuie să fie un număr' }),
    maxMileage: z.string().min(1, 'Kilometrajul maxim este obligatoriu').regex(/^\d+([.,]\d+)?$/, { message: 'Kilometrajul maxim trebuie să fie un număr' }),
